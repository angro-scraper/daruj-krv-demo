...
export default function KontrolniCentar({ onNav, uloga }: { onNav: (s: Screen) => void; uloga: Role }) {
  ...
  const allowed = (screen: Screen) => canOpen(uloga, screen)
  ...
  return (
    <PageWrap>
      ...
      {((kritZalihe > 0 && allowed('izvestaji')) || (ceka_odobravanje > 0 && allowed('odobravanje')) || (intGreska > 0 && allowed('api_integracije'))) && (
        <div className="rounded-xl border p-4 flex flex-col gap-2" style={{ background: C.burgundyBg, borderColor: C.burgundy + '40' }}>
          ...
          <div className="flex flex-wrap gap-3">
            {kritZalihe > 0 && allowed('izvestaji') && (
            ...
            {ceka_odobravanje > 0 && allowed('odobravanje') && (
            ...
            {intGreska > 0 && allowed('api_integracije') && (
      ...
      <ActionMap actions={actions} onOpenActions={allowed('akcije') ? () => onNav('akcije') : undefined} />

      <div className="grid lg:grid-cols-3 gap-5">
        {/* Zalihe */}
        <div className="lg:col-span-2">
          <Card>
            <CardHeader title="Zalihe krvi — trenutno stanje" subtitle="Sve filijale" action={allowed('izvestaji') &&
          ...
          <Card>
            ...
            {aktivnaAkcija ? (
              <div className="p-5 flex flex-col gap-4">
                ...
                {allowed('prijem_davalaca') && <button onClick={() => onNav('prijem_davalaca')} className="h-9 rounded-lg text-sm font-medium w-full" style={{ background: C.teal, color: C.white }}>
                  Otvori prijem →
                </button>}
              </div>
            ) : (
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        {(allowed('hijerarhija_audit') || allowed('audit_log')) && <Card>
          <CardHeader title="Poslednje aktivnosti (Audit)" action={
            <button onClick={() => onNav(allowed('audit_log') ? 'audit_log' : 'hijerarhija_audit')} className="text-xs font-medium" style={{ color: C.teal }}>Sve →</button>
          } />
          <div className="divide-y" style={{ borderColor: C.s100 }}>
